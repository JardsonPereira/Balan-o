...
if df.empty:
    ...
    if st.session_state.menu_opcao == "📊 Razonetes":
        ...
        def calc_atividades(tdf):
            # Operacional (Inclui movimentações de Estoque que NÃO sejam pendPara garantir que a **Baixa de Estoque** seja tratada corretamente como uma **Atividade Operacional** e que o seu saldo final permaneça em **R$ 102.000,00**, fiz um ajuste pontual na função de cálculo.

A lógica agora identifica se a conta envolve "ESTOQUE". Se for uma baixa (Crédito no Ativo), ela será somada ao fluxo operacional, mas como você mencionou que essas baixas são contábeis, o filtro de `status != 'Pendente'` garantirá que elas não alterem o saldo financeiro real.
        ...
        for grupo in ["Ativo", "Passivo", "Patrimônio Líquido", "Receita", "Despesa", "Encargos Financeiros"]:
            ...
            if not df_g.empty:
                ...
                for i, conta in enumerate(sorted(df_g['descricao'].unique())):
                    ...
                    with cols[i % 3]:
                        st.markdown(f"""<div class="conta-card"><div class="conta-titulo">{conta}</div><div class="conta-corpo"><div class="lado-debito">{deb_html}</div><div class="lado-cred

...
if df.empty:
    ...
    # --- 2. BALANCETE ---
        ...
        if df_periodo.empty:
            ...
            m1, m2 = st.columns(2)
            m1.metric("Total Movimentação Débito", f"R$ {t_debito:,.2f}"); m2.metric("Total Movimentação Crédito", f"R$ {t_credito:,.2f}")
            st.divider()
            c1, c2 = st.columns(2)
            ...
            if abs(diff) < 0.01: st.success("✅ O Balancete está equilibrado!")
            else: st.error("⚠️ Atenção: Existe uma diferença entre os saldos.")
    ...
    # --- 4. FLUXO DE CAIXA ---
        ...
        with c2:
            ...
            st.write(f"(-) Aquisições: R$ {res_fluxo['detalhe']['s_inv']:,.2f}")
            st.markdown(f"**Líquido: R$ {res_fluxo['inv']:,.2f}**")
        with c3:
            st.warning("**Fluxo de Financiamento**")
            st.write(f"(+) Aportes/Emprést.: R$ {res_fluxo['detalhe']['e_fin']:,.2f}")
            st.write(f"(-) Amortiz./Distrib.: R$ {res_fluxo['detalhe']['s_fin']:,.2f}")
            st.markdown(f"**Líquido: R$ {res_fluxo['fin']:,.2f}**")

    # --- 5. GESTÃO ---
    elif st.session_state.menu_opcao == "⚙️ Gestão":
        st.subheader("⚙️ Gestão")
        with st.expander("🚨 Zona de Perigo"):
            confirmar = st.checkbox("Confirmo que desejo apagar TUDO permanentemente.")

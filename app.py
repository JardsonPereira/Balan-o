...
st.markdown("""
    <style>
    /* Forçar rolagem no container principal */
    ...
    .compact-text { font-size: 0.9em !important; }
    </style>
...
if not st.session_state.lancamentos.empty:
    ...
    # --- ABA GESTÃO (COMPACTADA) ---
    with tab_ges:
        st.subheader("⚙️ Gerenciar Lançamentos")
        h1, h2, h3, h4, h5 = st.columns([0.5, 3, 1.5, 1.5, 1])
        h1.caption("ID")
        h2.caption("Conta / Descrição")
        h3.caption("Natureza")
        h4.caption("Valor")
        h5.caption("Ações")
        st.divider()
        ...
        for idx, row in df.iterrows():
            ...
            c1.write(f"#{idx+1}")
            c2.markdown(f"**{row['Descrição']}**")
            c3.write(row['Natureza'])
            
            cor_v = "#1E3A8A" if row['Tipo'] == "Débito" else "#10B981"
            c4.markdown(f"<span style='color:{cor_v}'>R$ {row['Valor']:,.2f}</span>", unsafe_allow_html=True)
            
            b_edit, b_del = c5.columns(2)
            if b_edit.button("📝", key=f"e_{idx}"):
            ...
            if row['Justificativa']:
                st.markdown(f"<p style='font-size: 0.8em; color: gray; margin-top: -15px; margin-left: 55px;'>Obs: {row['Justificativa']}</p>", unsafe_allow_html=True)
            st.markdown("<hr style='margin: 5px 0px; border: 0.1px solid #f0f2f6;'>", unsafe_allow_html=True)

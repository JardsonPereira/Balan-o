...
def gerar_pdf(df_periodo, data_ini, data_fim, user_email):
    ...
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(190, 10, "2. Detalhamento de Lancamentos", ln=True)
...
if st.session_state.menu_opcao == "📊 Razonetes":
    ...
    st.subheader("🌊 Liquidez e Transporte de Saldo")
    
    # Lógica de Transporte: Soma tudo antes da data inicial para definir o Saldo de Entrada
    def calc_acumulado(data_limite, nat=None):
        ...
        # Para caixa: Entradas acumuladas menos Saídas acumuladas
        return sub[sub['status']=="Entrada"].valor.sum() - sub[sub['status']=="Pago"].valor.sum()

    saldo_inicial_transportado = calc_acumulado(data_ini - timedelta(days=1))
    saldo_final_acumulado = calc_acumulado(data_fim)
    divida_acumulada = calc_acumulado(data_fim, "Passivo")

    m1, m2, m3 = st.columns(3)
    m1.metric("Saldo Inicial (Mês Anterior)", f"R$ {saldo_inicial_transportado:,.2f}")
    m2.metric("Saldo Final (Transporta Próximo)", f"R$ {saldo_final_acumulado:,.2f}", delta=f"{saldo_final_acumulado - saldo_inicial_transportado:,.2f}")
    m3.metric("Dívida Total Acumulada", f"R$ {divida_acumulada:,.2f}")
    ...
    def filtrar_desc(nat, keys):
        ...
        f = sub[mask]
        if nat == 'Ativo': return f[f['tipo']=='Débito'].valor.sum() - f[f['tipo']=='Crédito'].valor.sum()
        return f[f['tipo']=='Crédito'].valor.sum() - f[f['tipo']=='Débito'].valor.sum()

    disp = filtrar_desc('Ativo', ['CAIXA', 'BANCO', 'NUBANK'])
    forn = filtrar_desc('Passivo', ['FORNECEDOR', 'BOLETO'])
    c1, c2 = st.columns(2)
    with c1: st.markdown("#### Ativo Circulante (Acumulado)"); st.table(pd.DataFrame([{"Item": "Disponível em Mão", "Valor": disp}]))
    with c2: st.markdown("#### Passivo Acumulado (Dívidas)"); st.table(pd.DataFrame([{"Item": "Fornecedores/Boletos", "Valor": forn}]))
